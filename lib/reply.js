 ...
 * Reply class
 * @param {String}   data
 * @param {Function} callback that gets called when data is correctly parsed callback(err, this)
 * @param {String}   format of data. Currently supported 'xml', 'json'
 */
var Reply = exports.Reply = function(data, callback, format) {
    var self = this;
    // Merges response conent into objects variables, so it can be easily accessible
