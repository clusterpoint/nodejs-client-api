 ...
 */
var Connection = exports.Connection = function(connectionString, storageName, username, password, documentRootXpath, documentIdXpath) {
 ...
 * @param  {Function} callback that will be called when reqest recieves response
...
Connection.prototype.sendRequest = function (request, callback, format) {
    ...
    if (request && request.getRequestXml) {
        ...
        message = request.getRequestXml(self.documentRootXpath, self.documentIdXpath, ep, self.createXML);
    ...
    var client = net.connect(self.connectionOptions, function() { // 'connect listener'
    ...
    client.on('data', function(data) {
        ...
        if (received != 0 && received == content_len) {
            ...
            if (!self.connectionOptions.url) {
                ...
                new Reply(protobuf_reply[1], callback, format);
            } else {
                if (self.debug) console.log("Reply: " + reply);
                new Reply(reply, callback, format);
            }
        }
    });
