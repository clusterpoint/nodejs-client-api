...
Request.prototype.getRequestXml = function(documentRootXpath, documentIdXpath, envelopeParams, createXML) {
    ...
    for (var i in envelopeParams) {
        // This param is reset in user set envelope params
        if (this.envelopeParams[i]) continue;
    ...
    for (var i in this.envelopeParams) {
        for (var j in this.envelopeParams[i]) {
            if (createXML === true) {
                doc.ele("cps:" + i, this.envelopeParams[i][j]);
            } else {
                doc += "<cps:" + i + ">" + _.escape(this.envelopeParams[i][j]) + "</cps:" + i + ">";
            }
        }
    }
    if (createXML === true) {
...
/**
 * Sets envelope param for request (namespaced with cps)
 * @param {String} key     Parameter name
 * @param {Array|Object} values  Parameter value(s)
 * @param {bool} replace Should new value replace old ones
 */
Request.prototype.setEnvelopeParam = function(key, values, replace) {
    replace = replace || false;
    if (!_.isArray(values)) values = [values];
    if (replace || !this["envelopeParams"][key]) this["envelopeParams"][key] = [];
    for (var i in values) {            
        this["envelopeParams"][key].push(values[i]);
    }
}
...
Request.prototype.setFacet = function(value, replace) { 
    if (_.isArray(value)) {
        ...
        _.forEach(value, this.setFacet, this);
...
Request.prototype.setAggregate = function(value, replace) { 
    if (_.isArray(value)) {
        ...
        _.forEach(value, this.setAggregate, this);
    } else if (_.isString(value)) {
