...
Request.prototype.getRequestXml = function(documentRootXpath, documentIdXpath, envelopeParams, createXML) {
    ...
    for (var i in this.unknownParams) {
        ...
        for (var j in this.unknownParams[i]) {
            ...
            if (createXML === true) {
                doc.ele(root).att("xmlns:js", "JSON property").raw(val);
            } else {
                doc += "<" + root + " xmlns:js=\"JSON property\">" + val + "</" + root + ">";
            }
        }
    }
    if (createXML === true) {
 ...
 * @param {Object|String} val Object or string to return
 */
var ValueToXml = exports.ValueToXml = function(val, escape) {
    ...
    else return (escape) ? _.escape(val) : val;
}
...
var ObjectToXml = exports.ObjectToXml = function(obj) {
    ...
    for (var key in obj) {
        ...
        if (_.isArray(value)) {
            // If object is array process each element and add type to xml elements
            type = ' js:type="array"';
            ...
            throw new TypeError('"' + key + '" is of unsupported data type (' + (typeof value) + ')');
